...
imprimir os dados do cliente.*/
	...
	struct dados cadastrarCliente (char nome[],double dataNascimento[],double cpf[],char sexo[]);
	int main (){
		setbuf(stdin, NULL);
		setlocale(LC_ALL,"Portuguese");
		cadastrarCliente(cliente.nome, cliente.dataNascimento, cliente.cpf, cliente.sexo);
	...
	struct dados cadastrarCliente (char nome[],double dataNascimento[],double cpf[],char sexo[]){
		...
		fgets(cliente.nome,40,stdin);fgets(cliente.sexo,40,stdin);
