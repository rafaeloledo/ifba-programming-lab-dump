...
struct disc {
  char nome[TAM2], semestre[TAM3], professor[TAM2], alunos[TAM][TAM2],
      codigo[TAM3];
} disciplina[DISC];

void CadastroAluno(char nome[TAM2], char sexo, char dataNascimento[TAM3],
                   char cpf[TAM3], int matricula) {
  setlocale(LC_ALL, "Portuguese");
  int i = 0, cont1 = 0, cont2 = 0, cont3 = 0, cont4 = 0, cont5 = 0, dia, mes,
      ano, contcpf = 0, cont = 0;
  int h = 11, result = 0, result2, aux = 0, z = 1, a = 12, b = 1, resultado = 0,
      resultado2, e = 1;
  int l, k, j, t;
  do {
    if (cont == 0) {
      printf(
          "Digite uma das opções do menu secundário do aluno %d:\n1- Inserir "
    ...
    case 1: {
      ...
      if (((nome[0] - '0') >= 0 && (nome[0] - '0') <= 9) ||
          ((nome[0] - '0') >= -9 && (nome[0] - '0') <= 0) ||
          (strlen(nome) > TAM3)) {
        printf("\nNome inválido.");
      }
      cont1++;
    ...
    case 4: {
      contcpf = 0, cont4 = 0, h = 11, result = 0, aux = 0, z = 1, a = 12, b = 1,
      resultado = 0, e = 1;
  ...
  if (cont1 == 0 || cont2 == 0 || cont3 == 0 || cont4 == 0 || cont5 == 0) {
    printf("Aviso: Não foram digitados todos os dados ou não foi gerada a "
           "matrícula.\n");
  }
}

void CadastroProfessor(char nome[TAM2], char sexo, char dataNascimento[TAM3],
                       char cpf[TAM3], int matricula) {
  setlocale(LC_ALL, "Portuguese");
  int dia, mes, ano;
  int i = 0;
  ...
  do {
    if (cont == 0) {
      printf(
          "Digite uma das opções do menu secundário do professor %d:\n1- "
          "Inserir ou modificar nome\n2- Inserir ou modificar Sexo\n3- Inserir "
          "ou modificar Data de Nascimento\n4- Inserir ou modificar Cpf\n"
          "5- Gerar matricula\n6- Excluir cadastro do professor atual\n0- Sair "
          "do cadastro do professor %d\n",
          countp1, countp1);
      cont++;
    } else {
      printf("Digite uma das opções do menu secundário: ");
  ...
  if (cont1 == 0 || cont2 == 0 || cont3 == 0 || cont4 == 0 || cont5 == 0) {
    printf("Aviso: Não foram cadastrados todos os dados ou não foi gerada a "
           "matrícula.\n");
  }
}

void CadastroDisciplina(char nome[TAM2], char semestre[TAM3],
                        char professor[TAM2], char alunos[50][TAM2],
                        char codigo[TAM3]) {
  setlocale(LC_ALL, "Portuguese");
  ...
  do {
    if (cont == 0) {
      printf("Digite uma das opções do menu secundário da disciplina %d:\n1- "
             ...
             "Inserir ou modificar Professor\n4- Inserir ou modificar Alunos\n"
    ...
    case 3: {
      ...
      if (((professor[0] - '0') >= 0 && (professor[0] - '0') <= 9) ||
          ((professor[0] - '0') >= -9 && (professor[0] - '0') <= 0) ||
          (strlen(professor) > TAM3)) {
        printf("\nDisciplina inválida.");
      }
      cont3++;
      break;
    ...
    case 4: {
      cont4 = 0;
      do {
        printf("Digite qual o numero do aluno a se cadastrar digite um valor "
               "de 1 a 50: ");
        setbuf(stdin, NULL);
        scanf("%d", &countd11);
        ...
        if (((alunos[countd11 - 1][0] - '0') >= 0 &&
             (alunos[countd11 - 1][0] - '0') <= 9) ||
            ((alunos[countd11 - 1][0] - '0') >= -9 &&
             (alunos[countd11 - 1][0] - '0') <= 0) ||
            (strlen(alunos[countd11 - 1]) > TAM3)) {
          printf("\nAluno inválido.\n");
        } else {
          cont4++;
        }
    ...
    case 5: {
      ...
      if (codigo[3] != '.' ||
          (((codigo[0] - '0') >= 0 && (codigo[0] - '0') <= 9) ||
           ((codigo[0] - '0') >= -9 && (codigo[0] - '0') <= -1))) {
  ...
  do {
    if (cont == 0) {
      printf(
          ...
          "de Disciplinas, com nome do professor, que extrapolam 40 vagas\n"
          "0- Sair do menu\n");
      cont++;
    } else {
    ...
    case 3: {
      for (k = 0; k < DISC; k++) {
        if (strlen(disciplina[k].nome) > 1 &&
            strlen(disciplina[k].semestre) > 1 &&
            strlen(disciplina[k].professor) > 1 &&
            strlen(disciplina[k].codigo) > 1) {
          printf("Nome da disciplina %d: %s\n", k + 1, disciplina[k].nome);
          printf("Semestre da disciplina %d: %s\n", k + 1,
                 disciplina[k].semestre);
          printf("Professor da disciplina %d: %s\n", k + 1,
                 disciplina[k].professor);
    ...
    case 4: {
      ...
      scanf("%d", &d);
      if (strlen(disciplina[d].nome) > 1 &&
          strlen(disciplina[d].semestre) > 1 &&
          strlen(disciplina[d].professor) > 1 &&
          strlen(disciplina[d].codigo) > 1) {
        printf("Nome da disciplina %d: %s\n", d + 1, disciplina[d].nome);
        printf("Semestre da disciplina %d: %s\n", d + 1,
               disciplina[d].semestre);
        printf("Professor da disciplina %d: %s\n", d + 1,
               disciplina[d].professor);
        for (c = 0; c < TAM; c++) {
          if (strlen(disciplina[d].alunos[c]) > 1) {
            printf("Aluno %d da disciplina: %s", c + 1,
                   disciplina[d].alunos[c]);
          }
        }
    ...
    case 7: {
      for (k = 0; k < TAM; k++) {
        if (strlen(aluno[k].dataNascimento) == 10) {
          aluno[k].dia = (aluno[k].dataNascimento[0] - '0') * 10 +
                         (aluno[k].dataNascimento[1] - '0');
          aluno[k].mes = (aluno[k].dataNascimento[3] - '0') * 10 +
                         (aluno[k].dataNascimento[4] - '0');
          aluno[k].ano = (aluno[k].dataNascimento[6] - '0') * 1000 +
                         ((aluno[k].dataNascimento[7] - '0') * 100) +
                         ((aluno[k].dataNascimento[8] - '0') * 10) +
                         (aluno[k].dataNascimento[9] - '0');
    ...
    case 10: {
      for (k = 0; k < TAM; k++) {
        if (strlen(professor[k].dataNascimento) == 10) {
          professor[k].dia = (professor[k].dataNascimento[0] - '0') * 10 +
                             (professor[k].dataNascimento[1] - '0');
          professor[k].mes = (professor[k].dataNascimento[3] - '0') * 10 +
                             (professor[k].dataNascimento[4] - '0');
          professor[k].ano = (professor[k].dataNascimento[6] - '0') * 1000 +
                             ((professor[k].dataNascimento[7] - '0') * 100) +
                             ((professor[k].dataNascimento[8] - '0') * 10) +
                             (professor[k].dataNascimento[9] - '0');
          professor[k].data =
              professor[k].dia + professor[k].mes + professor[k].ano;
        }
      }
      for (k = 1; k <= TAM - 1; k++) {
        for (j = 0; j < TAM - k; j++) {
          if (strlen(professor[j].dataNascimento) > 1) {
            if (professor[j].data > 1) {
              if (professor[j].data > professor[j + 1].data) {
                ...
                strcpy(professor[j].dataNascimento,
                       professor[j + 1].dataNascimento);
      ...
      for (k = 0; k < TAM; k++) {
        if (strlen(professor[k].dataNascimento) > 1) {
          printf(
              "Professor: %s // Data de Nascimento: %s // Matrícula: %d //\n",
              professor[k].nome, professor[k].dataNascimento,
              professor[k].matricula);
        }
      }
      break;
    }
    case 11: {
      ...
      fgets(mes, 4, stdin);
      for (k = 0; k < TAM; k++) {
        if (dia[0] == aluno[k].dataNascimento[0] &&
            dia[1] == aluno[k].dataNascimento[1] &&
            mes[0] == aluno[k].dataNascimento[3] &&
            mes[1] == aluno[k].dataNascimento[4]) {
          printf("Aluno %d- %s é aniversariante.\n", k + 1, aluno[k].nome);
        }
      }
      for (k = 0; k < TAM; k++) {
        if (dia[0] == professor[k].dataNascimento[0] &&
            dia[1] == professor[k].dataNascimento[1] &&
            mes[0] == professor[k].dataNascimento[3] &&
            mes[1] == professor[k].dataNascimento[4]) {
    ...
    case 12: {
      ...
      for (k = 0; k < TAM; k++) {
        conter = 0;
        for (j = 0; j < TAM2; j++) {
          if (aluno[k].nome[j] == string[0] || aluno[k].nome[j] == string[1] ||
              aluno[k].nome[j] == string[2]) {
            conter++;
          }
          if (conter == 3) {
      ...
      for (k = 0; k < TAM; k++) {
        ...
        for (j = 0; j < DISC; j++) {
          if (strlen(aluno[k].nome) > 1) {
            if (j == DISC - 1) {
              if (conter < 3) {
                printf("Aluno %d- %s está matriculado em menos de 3 "
                       "disciplinas.\n",
                       k + 1, aluno[k].nome);
              }
            }
          ...
          for (t = 0; t < TAM; t++) {
            if (strlen(disciplina[j].alunos[t]) > 1 &&
                strlen(aluno[k].nome) > 1) {
    ...
    case 14: {
      printf("Se n printou nada eh pq n existem disciplinas com mais de 40 "
             "alunos.\n");
      conter = 0;
      for (k = 0; k < TAM; k++) {
        for (j = 0; j < TAM; j++) {
