...
struct dados cadastrarCliente (char nome[],char sexo,char dataNascimento[],char cpf[]);
int main (){
    setbuf(stdin, NULL);
    setlocale(LC_ALL,"Portuguese");
    cadastrarCliente(cliente.nome, cliente.sexo, cliente.dataNascimento, cliente.cpf);
    if(contN==0){printf("Nome validado com sucesso: %s", cliente.nome);}
    else{printf("Erro na validação.Nome é inválido.\n");}
    if(contS==0){printf("Sexo validado com sucesso: %c\n", cliente.sexo);}
    else{printf("Erro na validação.Sexo digitado é inválido.\n");}
    if(contNasc==0){printf("Data de nascimento validada com sucesso: %s\n", cliente.dataNascimento1);}
    else{printf("Erro na validação.Data de nascimento é inválida.\n");}
    if(contC==0){printf("Cpf validado com sucesso: %s\n", cliente.cpf);}
    else{printf("Erro na validação.Cpf digitado é invalido.");}
    return 0;
    system("PAUSE");
}
struct dados cadastrarCliente (char nome[],char sexo,char dataNascimento[],char cpf[]){
    ...
    printf("Digite o sexo do cliente. Opções: m e M para masculino, f e F para feminino, o e O para outro. Caso digite mais de um caractere o programa irá bugar.: ");
...
int validarNome(char nome[40]){
    if((cliente.nome[0]-'0')>=0 && (cliente.nome[0]-'0')<=9 || (cliente.nome[0]-'0')>=-9 && (cliente.nome[0]-'0')<=0 ){
        return contN++;
    }
...
int validarCPF(char cpf[20]){
    ...
    if((result*10)%11==cliente.cpf[12]-'0' || result2==cliente.cpf[12]-'0'){
        contador++;
    }
    else{
        return contC++;
    ...
    if((resultado*10)%11==cliente.cpf[13]-'0' || resultado2==cliente.cpf[13]-'0'){
        contador++;
    }
    else{
        return contC++;
